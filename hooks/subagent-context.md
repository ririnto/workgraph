...
- `blocked` means missing authority, information, tool access, compatible output format, or resource ownership prevents in-scope work.
- `failed` means required work or validation remains failed.
- `unknown` means available evidence cannot establish another outcome.
- State the outcome when the required final-result format supports it.

## Authority Boundary

- Run git commit or push only under an explicit git grant and after all required pre-action evidence passes.
...
- Keep ordinary progress and findings in the terminal result unless the dispatch explicitly requires live coordination.
